...
app.post("/api/login", async (req, res) => {
  ...
  const authenticationDetails = new AmazonCognitoIdentity.AuthenticationDetails({
    ...
    Password: hash + "D**"                 // Contraseña hasheada y modificada
  });

  // Datos del usuario para Cognito
  const userData = {
    Username: req.body.email,              // Email como identificador
    Pool: cognito,                         // User Pool definido previamente
  };
...
app.post("/api/signup", async (req, res) => {
  ...
  // Hashear la contraseña antes de enviarla a Cognito
  const hash = crypto.createHash('sha256').update(req.body.password).digest('hex');

  // Registrar el usuario en Cognito
  cognito.signUp(
    req.body.email,                     // Email usado como identificador
    hash + "D**",                       // Contraseña hasheada y modificada
